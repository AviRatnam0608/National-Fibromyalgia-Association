  ...
  useEffect(() => {
    ...
    const fetchSubmissions = async () => {
      try {
        ...
        const feedbackPromises = userSubmissions.map((submission) => {
          return getDocs(
            collection(db, `feedback/${submission.id}/comments`)
          ).then((querySnapshot) => {
        ...
        setSubmissions(
          submissionsWithFeedback.filter(
          ...
          <ul style={{ listStyleType: "none", padding: 0, margin: 0 }}>
            {submissions.map((submission) => (
              ...
              >
                ...
                <div>
                  <button
                    onClick={() => handleRevise(submission.id)}
                    style={{
                      marginRight: "5px",
                      background: "#ffc107",
                      color: "white",
